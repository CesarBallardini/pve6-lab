...
vm_template_debian_10() {
  ...
  # FIXME: ugly hack: el nombre del disco difiere entre ceph y local
  if [ "${STORAGE_POOL}" = "ceph-vm" ]
  then
    DISK0="${STORAGE_POOL}:vm-${VMID}-disk-0"
  else
    DISK0="${STORAGE_POOL}:${VMID}/vm-${VMID}-disk-0.qcow2"
  fi

  sudo qm set "${VMID}" -scsihw virtio-scsi-pci -virtio0 ${DISK0}
  ...
  #sudo qm set "${VMID}" --ipconfig0 ip=192.168.57.2/24,gw=192.168.57.1
  sudo qm template "${VMID}"
...
#lanza_vm_desde_template 9002 5002 deb10-vm5002 "--ipconfig0 ip=192.168.57.2/24,gw=192.168.57.1"
#lanza_vm_desde_template 9001 5005 ubu20-vm5005 "--ipconfig0 ip=192.168.57.5/24,gw=192.168.57.1"
