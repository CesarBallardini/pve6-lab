...
instala_requisitos_previos() {
  ...
  sudo -E apt-get install debconf-utils wget libguestfs-tools jq sshpass expect lshw ${APT_OPTIONS}
...
postfix	postfix/destinations	string	$pvenode_hostname, $myhostname, $pvenode_hostname.$pvenode_domain, localhost.$pvenode_domain, localhost
...
permite_root_login_con_passsword() {
...
echo root:${pvenode_root_password} | sudo chpasswd
